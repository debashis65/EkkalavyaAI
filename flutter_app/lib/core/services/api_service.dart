...
class ApiService {
  ...
  static const String aiBackendUrl = 'http://localhost:8000'; // AI Backend on port 8000
  static const bool isDevelopmentMode = true; // Set to false for production

  ApiService(this._dio, this._storage) {
    _dio.options.baseUrl = baseUrl;
    _dio.options.connectTimeout = const Duration(seconds: 5); // Shorter timeout for dev
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    if (isDevelopmentMode) {
      ...
      await _storage.write(key: 'auth_token', value: 'dev_token_${DateTime.now().millisecondsSinceEpoch}');
      ...
      return {
        ...
        'user': {
          ...
          'email': email,
          'role': email.contains('coach') ? 'coach' : 'athlete',
        },
