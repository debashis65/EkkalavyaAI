...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Video analysis API endpoints for authentic AI processing
  app.post("/api/analyze-video", async (req, res) => {
    try {
      const { sport, analysis_type, frames, timestamp, user_id, filename } = req.body;
      
      // Forward to AI backend for real analysis
      const aiResponse = await fetch('http://localhost:8000/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sport, analysis_type, frames, timestamp }),
      });
  ...
  app.post("/api/generate-drills", async (req, res) => {
    try {
      ...
      const response = await fetch('http://localhost:8000/recommend_drills', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sport, metrics, skill_level: athlete_level }),
      });
